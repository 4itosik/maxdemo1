...
// TestСквознойПроходСобытиеОтвет — та же проверка, что в паре на quicktype:
// webhook-сервер принимает событие, обработчик отвечает через клиент, роль
// Max Bot API играет httptest.Server. В сеть тест не ходит.
func TestСквознойПроходСобытиеОтвет(t *testing.T) {
	...
	maxAPI := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		outbound <- sent{r.URL.RawQuery, body}
