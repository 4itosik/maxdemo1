// Package maxclient — HTTP-клиент к Max Bot API поверх сгенерированных структур.
//
// Пример намеренно покрывает только два сценария из задачи: управление
// подписками на webhook и отправку сообщения. Всё остальное дописывается по
// тому же образцу — тела запросов и ответов уже есть в пакете maxapi, руками
// описывать нечего.
//
// Что стоит подсмотреть: тела запросов и ответов НЕ объявляются здесь заново,
// а берутся из maxapi — maxapi.SubscriptionRequestBody, maxapi.NewMessageBody,
// maxapi.SendMessageResult, maxapi.Error. В этом весь смысл кодогенерации:
// правка в *.tsp доезжает до клиента через `make gen-quicktype`, а не через
// синхронизацию структур руками.
package maxclient
...
// DefaultBaseURL — адрес Max Bot API из контракта (`servers` в openapi.MaxBotApi.yaml).
...
// access_token (securitySchemes.ApiKeyAuth), но query попадает в логи прокси и
// в access-логи — заголовок не попадает.
func (c *Client) do(ctx context.Context, method, path string, query url.Values, in, out any) error {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		// Тело может оказаться не-JSON (например, страница от прокси) —
		// тогда код и сообщение останутся пустыми, но статус сохранится.
